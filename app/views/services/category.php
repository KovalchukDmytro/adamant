		...
		<div class="row">
			<div class="col-sm-12">
						...
						<?php foreach ($content['models'] as $model) : ?>
							<div class="col-xs-10 col-xs-offset-1 col-sm-8 col-sm-offset-2 col-md-4 col-md-offset-0">
								<article class="widget widget--fluid">
									...
									<div class="widget-description"><?=$model->info->description ?></div><a class="widget-button widget-button--fluid" href="<?= Url::toRoute( '/services/'.$content['model']->alias.'/'.$model->alias ) ?>"><?= $this->params['go-to-the-service-index'] ?></a>
								</article>
							</div>
						<?php endforeach; ?>
					</div>
				...
				<section class="section section--services-post">
					<h3 class="section-title"><?= $content['model']->info->title ?></h3>
					<div class="text">
